...
public class JDBCExample {
		public static void main(String[] args) {
			...
			try {
				...
				// 115.90.212.22 < 계정따로 발급 받은사람들 서버컴 IP
...
//				System.out.println(conn);
				// oracle.jdbc.driver.T4Connection@2df3b89c
				...
				while(rs.next()) {
					...
					// 자바에서는 _ 사용 금지 파이널을 나타냄
					
					String empName = rs.getNString("EMP_NAME"); // 현재 행의 "EMP_NAME" 문자열 컬럼의 값을 얻어옴
					
					int salary = rs.getInt("SALARY"); // 현재 행의 "SALARY" 숫자(정수) 컬러의 값을 얻어옴
					
					// * java.sql.Date *
					Date hireDate = rs.getDate("HIRE_DATE");
					// -> java.util.Date도 가능하긴 하지만 java.sql.Date로 사용
