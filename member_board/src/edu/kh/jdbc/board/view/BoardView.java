...
public class BoardView {
	...
	private void selectAllBoard() {
		...
		try {
			...
			System.out.println("\n<<게시글 목록 조회중 예외 발생>>\n");
			e.printStackTrace(); // 예외 목록 확인 하는 구문
		}
	}
	...
	private void subBoardMenu(Board board) {
		...
		try {
			...
			// 댓글 1등록, 2수정, 3삭제 선택 시 각각의 서비스 메서드 종료 후 다시 서브메뉴 메서드 호출
			if(input > 0 && input < 5) {
				try {
	...
	private void insertComment(int bNo, int mNo) {
		...
		try {
			...
			// 하단에 만들어둔 "내용 입력 메서드" 활용 > 내용 입력 받기
	...
	private String inputContent() {
		String content = ""; // 빈 문자열 ( 콘텐트가 참조하는 값이 있지만, 내용이 없다 = 빈칸을 참조 하고 있다. )
	...
	private void updateComment(List<Comment> commentList, int memberNo) {
		...
		try {
			...
			for(Comment c : commentList) {
				...
				if(c.getCommentNo() == commentNo) { // 댓글 번호 일치 확인
					flag = false;
					
					if(c.getMemberNo() == memberNo) { // 회원 번호 일치 확인
	...
	private void insertboard() {
		try {
			...
			System.out.println("\n<<게시글 등록 중 예외 발생>>\n");
			e.printStackTrace(); // 예외 발생 호출 메서드
		}
	}
